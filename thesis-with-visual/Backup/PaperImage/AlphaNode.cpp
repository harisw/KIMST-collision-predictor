...
int AlphaNode::justTest(int TimeSlice)
{
	...
	if (EventResult.size() > 0) {
		...
		set<int> triggeredIds;
#ifdef SHOW_ALPHA_TRIGGERED

		queue<EventPtr> tempRes = EventResult;
		//if (isCQ) {		//ONLY OUTPUT RES for CQ Node
			while (!tempRes.empty()) {
				stringstream output_stream;
				int curObjID = tempRes.front()->getInt("objid");
				if (!triggeredIds.count(curObjID)) {
					triggeredIds.insert(curObjID);
					output_stream << "Object #" << curObjID << ", " << Utilities::translateTimeOnly(Utilities::getTimeSec()) << ", N" << CQNodeID << endl;
