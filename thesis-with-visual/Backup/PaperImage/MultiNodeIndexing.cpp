...
unordered_map<string, int> MultiNodeIndexing::hashCombination;

int MultiNodeIndexing::checkAttrMap(string attrName)
{
	string name = Utilities::ToLower(attrName);
	auto got = attributes_map.find(name);
...
int MultiNodeIndexing::returnFinalNode(EventPtr ep)
{
	...
	//this is based on hari's recommendation
	//unordered_map<int, int> fetchedNode_map = stabEventMap(ep);
	//for (auto n : combinationMap) {
	//	int match_sum = 0;
	//	for (auto f_n : n.second) {
	//		auto res = fetchedNode_map.find(f_n);
	//		if (res != fetchedNode_map.end())
	//			match_sum++;
	//	}
	...
	string str;
	char Buff[10];
	for (auto num : fetchedNode) {
		str += _itoa(num, Buff, 10);
		str += ",";
	}
	str = str.substr(0, str.size() - 1);
	auto res = hashCombination.find(str);
	if (res != hashCombination.end()) { // match
		return res->second;
	}
	else {
		return -1;
...
void MultiNodeIndexing::createCombination()
{
	...
	for (auto node_now : finalNodeIsTheNodeWithoutWindow) {
		...
		//lets try with the hashing stuff
		string str;
		char Buff[10];
		for (auto num : traverse_made) {
			str += _itoa(num, Buff, 10);
			str += ",";
		}
		//remove last coma then save
		str = str.substr(0, str.size() - 1);
		hashCombination[str] = node_now->getID();
...
vector<pair<int, pair<int, int>>> MultiNodeIndexing::adjustRange(vector<Node*> tempNodeVector)
{
	...
	if (tempPair.size() > 1) {
		...
		for (int i = 0; i < tempPair.size() - 1; i++) {
			finalRes.push_back({ tempPair[i].first, {tempPair[i + 1].second.second, tempPair[i].second.second} });
		}
	}
	//push the last
	if (tempPair.size() > 0)
...
unordered_map<int, int> MultiNodeIndexing::stabEventMap(EventPtr ptr)
{
	RectHelper r;

	//ah, just iterate it
	for (auto a : ptr->getMap()) {
		int checker = checkAttrMap(a.first);

		if (checker != -1)
			r.searchAndSet(checker, ptr->getString(a.first));
		//r.(checker, ep->getString(a.first));
	}

	//return multiDimensionalTree.Search_vec(r.max, r.min, NULL, NULL);
	return multiDimensionalTree.Search_map(r.max, r.min, NULL, NULL);
}
