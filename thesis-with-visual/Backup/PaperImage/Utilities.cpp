...
unsigned long Utilities::start_cycle_time = 0;
...
long long Utilities::getTimeSec()
{
...
string Utilities::translateTime(long long epochSecond)
{
	return asctime(gmtime(&epochSecond));;
}

string Utilities::translateTimeOnly(long long epochSecond)
{
	string s = asctime(gmtime(&epochSecond));;
	...
	string& limit, string& condition, string& product)
{
	...
	if ((index = expression.find("then")) < 0) {
		...
		if (index = expression.find(",") > 0) { //might be distance(a,b)<10, or plot_dist
			...
			else if (specialOperation == "sonar_dist") {
				leftSource = expression.substr(leftBr + 1, expression.find_last_of(",") + 1);

				//last of the last
				index = expression.find("config(");
				if (index > 0) {
					//sorry this is fixed ._.
					int rightComa = expression.find_last_of(",");
					rightBr = expression.find_last_of(")");
					//vector<string> st_v = Utilities::splitDelim(expression.substr(rightBr + 2, expression.length() - (rightBr + 3)),",");
					rightSource = expression.substr(rightComa + 1, (expression.length() - rightComa - 2));

					limit = expression.substr(index + 6, rightComa - index - 6);
			...
			else if (specialOperation == "radar_dist") {
				leftSource = expression.substr(leftBr + 1, expression.find_last_of(",") + 1);

				//last of the last
				index = expression.find("point(");
				if (index > 0) {
					//sorry this is fixed ._.
