...
bool ReteNet::is_execution_started = false;
int ReteNet::CQGlobalID = 0;

queue< string > ReteNet::current_progress;
...
#pragma region NewSpatialRegion
	for (auto str : observed_obj_dict) {
		...
		for (; result.size() > 0; result.pop()) {
			...
			if (!stabbed_node_boost.empty()) {
				for (int j = 0; j < stabbed_node_boost.size(); j++) {
					stringstream output_stream;
					Node* stabbedNode = getNodeFromId(stabbed_node_boost[j]);
					ReteNet::triggered_node_ID[stabbed_node_boost[j]] = true;

					output_stream << "Object #" << result.front()->getId() << ", " << Utilities::translateTimeOnly(Utilities::getTimeSec()) << ", N" << stabbedNode->CQNodeID << endl;
...
void ReteNet::checkTemporalQueue(int TimeSlice, int TimeNow)
{
	...
	void ReteNet::clearRules()
	{
		ReteNet::alphaListIDDictionary.clear();
		ReteNet::betaListIDDictionary.clear();
		ReteNet::NodeList.clear();
	}
