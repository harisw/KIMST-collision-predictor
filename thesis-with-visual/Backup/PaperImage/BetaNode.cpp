...
int BetaNode::justTest(int TimeSlice)
{
	...
	if (win != NULL && EventResult.size() > 0 && specialOperation == "") {
		...
		//ClearResult();
		EventResult = win->getFinalRes(triggeredIds);
	...
	//If it doesn't have, maybe this is the end?
		...
		if (!triggeredIds.empty()) {
			for (auto f : triggeredIds) {
				output_stream << "Object #" << f << ", " << Utilities::translateTimeOnly(Utilities::getTimeSec()) << ", N" << CQNodeID << endl;
				queue_ev.push(output_stream.str());
			}
		}
		//output_stream <<"Object #"<<EventResult.front()->getInt("objid")<< ", "<< Utilities::translateTimeOnly(Utilities::getTimeSec()) << ", R" << SuperNodeID << endl;
		//queue_ev.push(output_stream.str());
	...
	if (EventResult.size() > 0) {
		ReteNet::triggered_node_ID[CQNodeID] = true;
#ifdef INDEX_ON
