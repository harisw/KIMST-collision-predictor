...
void SlidingWindow::refresh(queue<EventPtr>& eventRes, set<int>& objIds)
{
	if (eventRes.size() <= 0)
		return;
	...
	while (!eventRes.empty()) {
		objIds.insert(eventRes.front()->getInt("objid"));
		if (eventRes.front()->map_size() < -1) {
			eventRes.pop();
			continue;
		}
...
queue<EventPtr> SlidingWindow::getFinalRes(set<int>& objIds)
{
	refresh(win_res_buffer, objIds);
