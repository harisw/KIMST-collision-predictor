...
public:
    ...
    unordered_map<int, int> Search_map(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context);
...
protected:
    ...
    bool Search_map(Node* a_node, Rect* a_rect, unordered_map<int, int>& a_map, t_resultCallback a_resultCallback, void* a_context);
...
RTREE_TEMPLATE
unordered_map<int, int> RTREE_QUAL::Search_map(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context)
{
#ifdef _DEBUG
    for (int index = 0; index < NUMDIMS; ++index)
    {
    ...
    unordered_map<int, int> map_found = {};
    Search_map(m_root, &rect, map_found, a_resultCallback, a_context);

    return map_found;
}

RTREE_TEMPLATE
bool RTREE_QUAL::Search_map(Node* a_node, Rect* a_rect, unordered_map<int, int>& a_map, t_resultCallback a_resultCallback, void* a_context)
{
    ASSERT(a_node);
    ...
    if (a_node->IsInternalNode())
    {
        ...
        for (int index = 0; index < a_node->m_count; ++index)
        {
            ...
            {
                if (!Search_map(a_node->m_branch[index].m_child, a_rect, a_map, a_resultCallback, a_context))
                {
